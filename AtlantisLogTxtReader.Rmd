...
#AtlantisRunFolder = "C:/nospaces/trunk/gomatlantis_trunk_ap2017_null/"
...
PlotAsTif=FALSE        #if false then it produces PDFs. if true it produces TIFs
tifheight=8
#----------------------------------------------------------------------------------
...
#READ LOG.TXT FILE BY STRING INTO VARIABLE 'X'
  ...
  for (i in 2:length(data))
  {
    ...
    if(current_row[[1]][1] == "Time:" & prior_row[[1]][1] != "Time:" & current_row[[1]][4] != "catch" & current_row[[1]][6] != "tier" & current_row[[1]][3] != "year")
    {
...
```{r create pdf or tif plots, echo=FALSE}
  ...
  if (PlotAsTif == FALSE) { 
  Outfile <- paste(AtlantisRunFolder,"Biomass_Plots.pdf",sep="")
  pdf(Outfile)
  } 
  ...
  { 
    ...
    if (PlotAsTif == TRUE) {
      Outfile = paste(AtlantisRunFolder,"Biomass",NumberOfFigs,".tif",sep="")    
  ...
  if (PlotAsTif == FALSE){  
  Outfile <- paste(AtlantisRunFolder,"Res_N_Plots.pdf",sep="")
  pdf(Outfile)  
  }
  ...
  while (k <= length(Group_Names[ , 1]))
  {
      ...
      if (PlotAsTif == TRUE) {
      Outfile = paste(AtlantisRunFolder,"Res_N",NumberOfFigs,".tif",sep="")    
      tiff(filename = Outfile,width=6, height=tifheight, units="in", res=600, compression = "lzw")
    }
  ...
  if (PlotAsTif == FALSE){  
  Outfile <- paste(AtlantisRunFolder,"Str_N_Plots.pdf",sep="")
  pdf(Outfile)  
  }
  ...
  while (k <= length(Group_Names[ , 1]))
  {
    ...
    if (PlotAsTif == TRUE) {
      Outfile = paste("Str_N",NumberOfFigs,".tif",sep="")    
      tiff(filename = Outfile,width=6, height=tifheight, units="in", res=600, compression = "lzw")
    }
 ...
 # windows(width=8.5,height=11)
  ...
  if (PlotAsTif == FALSE){  
  Outfile <- paste(AtlantisRunFolder,"Density_Plots.pdf",sep="")
  pdf(Outfile)  
  }
  ...
  {
    if (PlotAsTif == TRUE) {
      Outfile = paste(AtlantisRunFolder,"Numbers",NumberOfFigs,".tif",sep="")    
      tiff(filename = Outfile,width=6, height=tifheight, units="in", res=600, compression = "lzw")
    }
...
Btimesteps<-as.numeric(as.character(Btimesteps))
 ...
 if (PlotAsTif == FALSE) { 
  Outfile <- paste(AtlantisRunFolder,"HistoricalFits.pdf",sep="")
  pdf(Outfile)
  }
...
if (plotsfound %% (rows*columns) == 1 && plotthis == TRUE) {
  ...
  NumberOfFigs = NumberOfFigs + 1
  Outfile = paste(AtlantisRunFolder,"Historical",NumberOfFigs,".tif",sep="")    
  tiff(filename = Outfile,width=6, height=tifheight, units="in", res=600, compression = "lzw") 
  par(mfcol=c(rows,columns))
  par(mar=c(2,2,2,2))
  par(omi=c(.5,.5,0,.5)) 
